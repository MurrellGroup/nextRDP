...
| Manual workflow | Browser implementation | Session 15 boundary |
| --- | --- | --- |
...
| Select topology, methods, window, p cutoff, correction, and breakpoint polishing | Circular defaults, RDP window 30, enabled MaxChi window 70, enabled CHIMAERA window 60, enabled GENECONV G-scale 1/one overlap, enabled no-window 3SEQ, p = 0.05, project correction, supplied BURT flag enabled; the method panel distinguishes discovery from confirmation | RDP plus ordinary triplet MaxChi, CHIMAERA, ignored-indel KA GENECONV, and exact/Siegmund 3SEQ discovery are active; remaining methods/modes stay locked or explicitly pending |
| Primary scan across eligible triplets | Incremental combined RDP/MaxChi/CHIMAERA/GENECONV/3SEQ worker scan with method-aware progress and cancellation; a lazy cursor selects either every exploratory triplet or only one-query/two-cross-group-reference triplets; later-round 3SEQ calls trim missing/erased runs and re-evaluate both orientations as supplied | Source-only and unvalidated; 3SEQ manual-envelope/full late event-catalogue modes, GENECONV permutation/manual/alternative-indel/full late-event modes, MaxChi manual-doublet/permutation, CHIMAERA permutation/full late event reconstruction, and other methods remain unavailable; bounded MaxChi/CHIMAERA strongest-peak, ordinary-kernel GENECONV, and two-orientation 3SEQ Findall rechecks are active |
...
| Reconcile signals into an event | Two shared original sequences plus strict >30% symmetric tract overlap across RDP, MaxChi, CHIMAERA, GENECONV, and 3SEQ signals; any enabled method may anchor the strongest event and coordinates | MaxChi/CHIMAERA/GENECONV/3SEQ preliminary roles feed the same late role arbitration; cross-method native ordering still needs golden validation |
...
| Save often | Review-header checkpoint plus export-page project download; a dirty/current indicator, tab-exit warning, and destructive-replacement guard track whether the latest scan/review state was downloaded; `v1alpha15` retains alignment, analysis scheme/reference groups, discovery method and MaxChi/CHIMAERA/GENECONV/3SEQ traces including split/Findall state, round factors/counters, fragment provenance, active RDP/GENECONV/MaxChi/CHIMAERA/3SEQ rechecks, final membership stages, groups, edits, and decisions | Alpha schema, not an `.rdp5` replacement; pre-v13 imports keep GENECONV disabled, pre-v14 imports keep 3SEQ disabled, and v14 cannot claim v15 split evidence |
...
- Method panel: RDP, GENECONV, MAXCHI, CHIMAERA, and 3SEQ retain their source method-major order;
  all five ordinary triplet discovery routes are active. 3SEQ split and late Findall are active;
